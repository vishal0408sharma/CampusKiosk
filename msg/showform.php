...
$operation="";
	...
	if($operation=="del")
	{
		$tempqry = "delete from $table where msg_id='$cx'";
	...
	if($operation=="upd")
	{
		$tempqry = "select readcheck from $table  where msg_id='$cx'";
		$response = mysql_query($tempqry);
		$row = mysql_fetch_array($response);
		if($row['readcheck']=='y')
		$tempqry = "Update $table set readcheck='n' where msg_id='$cx'";
		else
		$tempqry = "Update $table set readcheck='y' where msg_id='$cx'";
	$response = mysql_query($tempqry);
	if($response!=true)
	{
	echo "Could not Delete :".mysql_error();	
...
<?php
	...
	{
$enr=$_SESSION["enroll"];
$query = "select * from $table where receiver_id='$enr'";
}
else
	{
	$tempid=$_SESSION["id"];
$query = "select * from $table where receiver_id='$tempid'";
}
